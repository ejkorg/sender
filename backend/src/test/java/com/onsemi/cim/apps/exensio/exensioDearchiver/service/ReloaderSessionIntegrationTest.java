...
public class ReloaderSessionIntegrationTest {
    ...
    @Autowired
    private com.onsemi.cim.apps.exensio.exensioDearchiver.service.SenderService senderService;
    ...
    com.onsemi.cim.apps.exensio.exensioDearchiver.service.SenderService.EnqueueResultHolder result = senderService.enqueuePayloadsWithResult(session.getSenderId(), payloadIds, session.getSource());
    assertThat(result).isNotNull();
    assertThat(result.enqueuedCount).isGreaterThan(0);

    // assert sender_queue entries created locally (enqueued)
    java.util.List<com.onsemi.cim.apps.exensio.exensioDearchiver.entity.SenderQueueEntry> q = senderQueueEntryRepository.findBySenderIdAndStatusOrderByCreatedAt(Integer.valueOf(params.get("senderId")), "NEW", org.springframework.data.domain.PageRequest.of(0, 1000));
